# APP System Reference 2

> Definitive internal map of the current `demo-raffle-v1` Twitch raffle implementation (snapshot: 2025‑12‑05). Update this file whenever authentication, Twitch sync, session, or raffle behavior changes.
...
- Twitch provider scopes: `openid user:read:email user:read:follows moderator:read:followers channel:read:subscriptions bits:read` with PKCE + state. `allowDangerousEmailAccountLinking` is enabled to merge returning users by email.
...
- **Viewer weight endpoint (`/api/weight/me`)** – Canonical UI entry point. Reads the cached `User` row; if `needsResync = true` or `lastUpdated` is older than `STALE_WEIGHT_MAX_AGE_MS` (6h), calls `syncUserFromTwitch` and then returns the DB-driven breakdown/settings.
...
### 8.1 Weight Calculation Source of Truth (`lib/weight-settings.ts`)

- `getEffectiveSubMonths(user)` normalizes Helix subscription data so active subscribers are treated as having at least one month (Helix `/subscriptions` currently returns `0`).
...
- The same helpers are used by `/api/weight/me`, `/api/leaderboard`, `/api/admin/dashboard`, and any admin tooling to avoid duplicate math.
...
│  ├─ components/                  → Shared UI pieces (MyStatusCard, WeightInfoModal, AdminUserTable, TwitchLogin, etc.)
│  └─ api/
...
│  ├─ leaderboard-data.ts, admin-data.ts, weight-settings.ts
│  ├─ twitch-api.ts, twitch-oauth.ts
│  ├─ prisma.ts, env.ts            → Infra helpers
│  ├─ rate-limit.ts, draw-lock.ts, follow-status.ts, etc.
...
### 12.1 Operational Scripts & Tooling

- `npm run prisma:migrate:dev` – generate migration for schema changes (commit the SQL).
- `npm run prisma:migrate:deploy` – must run in every deploy pipeline **before** the Next.js app boots; keeps DB/schema in sync.
- `npm run twitch:diagnose` – support CLI that loads broadcaster credentials, hits `helix/channels/followers` + `helix/subscriptions`, and reports missing scopes/status codes.
- `npm run twitch:eventsub:provision` – provisions/updates EventSub subscriptions defined in `scripts/eventsub-provision.mjs` (fixed template literals, `dotenv/config`, canonical callback URL, structured error logging).
- `npm run lint` – ESLint 9 over the full monorepo (App Router + scripts).
